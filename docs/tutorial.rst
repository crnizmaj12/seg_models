...
**Seg-Models** is python library with Neural Networks for
`Image
...
Quick start
~~~~~~~~~~~
Since the library is built on the Keras framework, created seg-model is just a Keras Model, which can be created as easy as:
...
.. code:: python
   ...
   from seg_models.losses import bce_jaccard_loss
   from seg_models.metrics import iou_score
...
Same manimulations can be done with ``Linknet``, ``PSPNet`` and ``FPN``. For more detailed information about models API and  use cases `Read the Docs <https://domain.io/en/latest/>`__.
...
.. code-block:: python
    ...
    from seg_models import Unet
    from seg_models.utils import set_trainable
